...
const BRIDGE_PATH = pathResolve(BRIDGE_DIR, "h2-bridge.mjs");
...
function flattenMessageContent(content: OpenAIMessage["content"]): string {
...
function putBlob(blobStore: Map<string, Uint8Array>, data: Uint8Array): Uint8Array {
...
function parseMessages(messages: OpenAIMessage[]): ParsedMessages {
  ...
  for (const msg of nonSystem) {
    if (msg.role === "tool") {
      ...
      pendingUser = flattenMessageContent(msg.content);
...
function buildCursorRequest(
  ...
  for (const turn of turns) {
    ...
    const userMsgBytes = putBlob(blobStore, toBinary(UserMessageSchema, userMsg));
    ...
    if (turn.assistantText) {
      ...
      stepBytes.push(putBlob(blobStore, toBinary(ConversationStepSchema, step)));
    }
    ...
    // ConversationStateStructure.turns holds blob IDs, not the turn payload.
    turnBytes.push(putBlob(blobStore, toBinary(ConversationTurnStructureSchema, turnStructure)));
  }
...
function explainCursorPolicyError(message: string): string {
  if (/Fable 5's data retention policy/i.test(message) || /acknowledge Fable/i.test(message)) {
    return (
      `${message} ` +
      "Fable 5 requires leaving Cursor's zero-data-retention / no-training mode. " +
      "Open the Cursor app → Settings → Models, select Fable 5, and accept the data-retention notice. " +
      "This proxy cannot silently accept that policy for you."
    );
  }
...
function extractConnectErrorDetail(error: unknown): string | null {
  ...
  for (const item of details) {
    ...
    const nested =
      (debug && typeof debug === "object"
        ? (debug as Record<string, unknown>).details
        : undefined) ?? rec.details;
    if (nested && typeof nested === "object") {
      const info = nested as Record<string, unknown>;
...
function parseConnectEndStream(data: Uint8Array): Error | null {
  try {
    ...
    if (error) {
      ...
      const nested = extractConnectErrorDetail(error);
      const raw =
        nested ||
        (typeof error.message === "string" && error.message !== "Error"
          ? error.message
          : "Unknown error");
      return new Error(`${code}: ${explainCursorPolicyError(raw)}`);
    }
    return null;
  } catch {
...
function handleKvMessage(
  ...
  if (kvCase === "getBlobArgs") {
    ...
    const blobData =
      blobStore.get(blobIdKey) ??
      blobStore.get(Buffer.from(blobId).toString("utf8"));
    if (!blobData) {
      console.error(`[proxy] missing blob ${blobIdKey.slice(0, 16)} store=${blobStore.size}`);
    }
